...
export default async function handler(req, res) {
  ...
  const INTRO_STYLES = [
    '推荐文章时不要用"为你推荐""给你找了"这类固定开头，试试用自然的过渡，像"说到这个，有篇分享写得挺好的"或"正好想起一篇"，然后直接亮出 #ID，不要复述文章内容。',
    '推荐文章时最多用一句话简短引出，再亮出 #ID，比如"有个学姐写了她跨保 AI 的经历，挺燃的 #15"，剩下的让用户点开链接看，不要替她讲完。',
    '推荐文章时可以用提问引入，比如"你有没有看过 XXX 的分享？跟你现在的情况挺像"，然后给 #ID 链接，不展开文章细节。',
    '推荐文章时可以用"我印象比较深的是…"或"之前有个学长/学姐写了…"来引入，点到为止后直接给 #ID，不摘抄内容。',
    '推荐文章时一句话带过亮点即可，比如"有篇写保研时间线的挺实用 #ID"，不要把文章里的步骤/数据列出来。',
  ];
...
- 自然融入对话，最多用一句话说明为什么推荐，然后标注文章 ID（格式：#ID）
- 一次推荐 1-3 篇，不要堆砌
...
【关于推荐时如何给出文章——务必严格遵守】
- **绝对不要把公众号文章的正文内容复述、改写或"生成一个文字版"出来**。用户要的是文章本身的链接，不是你转述的内容。
- **尽量减少对文章具体内容的评论和引用**：不要大段摘抄文章里的观点、步骤、数据或金句。点到为止即可。
- 推荐时只需：一句话简短引出（甚至不评论也行）+ 标注 #ID，剩下的让用户点开链接自己看。
- 标注了 #ID 后，前端会自动渲染成可点击的文章卡片/链接，所以你不需要、也不要再用文字把文章内容铺开讲。
