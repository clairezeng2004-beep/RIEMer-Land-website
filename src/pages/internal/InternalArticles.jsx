...
import {
  ...
  CheckSquare, Sparkles, Eye, ClipboardPaste, ImagePlus,
} from 'lucide-react';
...
export default function InternalArticles() {
  ...
  const [savingReadNumId, setSavingReadNumId] = useState(null);
  ...
  useEffect(() => {
    if (!showReadNumModal) return;
    setReadNumDraft((prev) => {
      const next = { ...prev };
      allArticles.forEach((a) => {
        next[a.id] = a.readNum != null ? String(a.readNum) : '';
      });
      return next;
    });
  }, [allArticles, showReadNumModal]);

  const saveCardReadNum = async (article, rawValue) => {
    if (!article || savingReadNumId) return;
    const next = Number(String(rawValue || '').replace(/[^0-9]/g, '')) || 0;
    const prev = Number(article.readNum ?? 0) || 0;
    if (next === prev) return;

    setSavingReadNumId(article.id);
    try {
      const result = await updateArticle(article.id, { readNum: next }, user?.id);
      if (!result?.success) {
        alert(`阅读量保存失败：${result?.error || '数据库没有确认更新成功，请稍后重试。'}`);
      }
    } catch (err) {
      alert(`阅读量保存失败：${err?.message || '未知错误'}`);
    } finally {
      setSavingReadNumId(null);
    }
  };
    ...
    <div className="ia-list-page">
      <div className="container">
        <div className="ia-list__header">
          <div>
            <h1>
              <FileText size={28} /> <EditableText as="span" value={ia.pageTitle || '公众号长文分享归档'} configKey="internalArticles.pageTitle" onChange={v => updateIA('pageTitle', v)} />
            </h1>
            ...
            const commonInner = (
              <div className="ia-card__body">
                ...
                <div className="ia-card__footer">
                  ...
                  {canManageArchive && (
                    <label
                      className="ia-card__readnum"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <Eye size={12} />
                      <input
                        key={`${article.id}-${article.readNum ?? 0}`}
                        type="text"
                        inputMode="numeric"
                        pattern="[0-9]*"
                        ...
                        onBlur={(e) => saveCardReadNum(article, e.currentTarget.value)}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') e.currentTarget.blur();
                          if (e.key === 'Escape') {
                            e.currentTarget.value = article.readNum || '';
                            e.currentTarget.blur();
                          }
                        }}
                        aria-label={`编辑「${article.title}」阅读量`}
                      />
                    </label>
                  )}
